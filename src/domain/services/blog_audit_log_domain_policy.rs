//! Domain policy for BlogAuditLog aggregate
//!
...
/// Domain policy for BlogAuditLog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<BlogAuditLog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BlogAuditLogDomainPolicy = PermitAllPolicy<BlogAuditLog>;
