//! Domain policy for Blog aggregate
//!
...
/// Domain policy for Blog — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<Blog>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type BlogDomainPolicy = PermitAllPolicy<Blog>;
