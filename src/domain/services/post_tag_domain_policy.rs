//! Domain policy for PostTag aggregate
//!
...
/// Domain policy for PostTag — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<PostTag>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type PostTagDomainPolicy = PermitAllPolicy<PostTag>;
