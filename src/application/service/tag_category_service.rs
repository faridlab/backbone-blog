//! Application service for TagCategory entities
//!
//! Generated by metaphor-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `tag_category_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::domain::entity::TagCategory;
use crate::infrastructure::persistence::TagCategoryRepository;
use crate::presentation::dto::{CreateTagCategoryDto, UpdateTagCategoryDto};

/// Application service for TagCategory entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `tag_category_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct TagCategoryServiceCustom {
///     inner: Arc<TagCategoryService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type TagCategoryService = GenericCrudService<
    TagCategory,
    CreateTagCategoryDto,
    UpdateTagCategoryDto,
    TagCategoryRepository,
>;

// <<< CUSTOM
// END CUSTOM
